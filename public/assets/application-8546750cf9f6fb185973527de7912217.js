...
}(window, jQuery));app.controller('AttachmentCtrl', AttachmentCtrl);
AttachmentCtrl.$inject = [];
function AttachmentCtrl() {
}
app.controller('CentreHomeController', CentreHomeController);
CentreHomeController.$inject = [
  '$scope',
  'Micropost',
  'PhotoService',
  'VideoUploadService',
...
function CentreHomeController($scope, Micropost, PhotoService, VideoUploadService, VoteUrlService, $window, CommentUrlService, $upload, GetInterestsService, PhotoUploadService, FileUploadService, $rootScope) {
  ...
  $scope.post = function () {
    if ($scope.data != null) {
      ...
      var post = new Micropost({ content: micropost });
      post.$save();
  ...
  $scope.micropostComment = function (micropost) {
    var comment = { content: micropost.commentIt };
  ...
  $scope.attachmentComment = function (attachment) {
    var comment = { content: attachment.commentIt };
  ...
  $scope.videoComment = function (vadeo) {
    var comment = { content: vadeo.commentIt };
...
function ActivityCtrl($scope, ActivityIndexService, ActivityOtherUserService, VoteUrlService, CommentUrlService) {
      ...
      ActivityIndexService.getActivities(counter).success(function (data) {
        ...
        angular.forEach(activities, function (value, key) {
          ...
          if (value.trackable_type != 'Comment') {
            ActivityOtherUserService.getFeed(value.user_id, value.trackable_id, value.trackable_type).success(function (data) {
  ...
  $scope.comment = function (userDetail) {
    var comment = { content: userDetail.commentIt };
...
function ChatCtrl($scope, MessageService) {
  ...
  $scope.sendMessage = function () {
    ...
    var message = { content: $scope.data.message };
...
PlaceNewController.$inject = [
  '$scope',
  'PlaceVideoUploadService'
];
...
PlaceUnsignedCtrl.$inject = [
  '$scope',
  'UnsignedPlaceServices'
];
...
function ActivityIndexService($http) {
  return {
    getActivities: function (offsets) {
    ...
    attachFile: function ($files, myModelObj, upload, interestIds, closeVideoModal) {
      ...
      for (var i = 0; i < $files.length; i++) {
        ...
        upload = $upload.upload({
          ...
          progress: function (evt) {
            ngProgress.start();
            ngProgress.set(parseInt(100 * evt.loaded / evt.total));
          }
        }).success(function (data, status, headers, config) {
          ngProgress.complete();
          console.log(data);
...
$('#post_interest_tokens').tokenInput('/interests.json', { preventDuplicates: true }, { prePopulate: $('#post_interest_tokens').data('load') });
$('#user_workplace_tokens').tokenInput('/workplaces.json', {
  token_limit: 1,
...
$(function () {
  ...
  $('#query').autocomplete({ source: '/search_suggestions' });
    ...
    '$routeProvider',
    function ($routeProvider) {
      $routeProvider.when('/', { controller: 'HomeController' }).otherwise({ redirectTo: '/' });
    }
  ]).config([
    '$locationProvider',
